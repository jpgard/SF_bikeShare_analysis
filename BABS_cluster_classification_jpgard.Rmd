...
boxplot(tripdata_final$Duration, main = "BABS Trip Duration")
hist(filter(tripdata_final, Duration <= 3600)$Duration, main = "Histogram of Duration for Rides < 1 Hour", xlab = "Duration")

par(mfrow=c(2,2), cex.main = 1)
hist(tripdata_final$Mean.TemperatureF, main = "Distribution of Daily Mean Temperatures", xlab = "Temperature in ºF")
hist(tripdata_final$Mean.Humidity, main = "Distribution of Daily Mean Humidity", xlab = "Percent Humidity")
hist(tripdata_final$Mean.VisibilityMiles, main = "Distribution of Daily Visibility", xlab = "Visibility in Miles")
hist(tripdata_final$Mean.Wind.SpeedMPH, main = "Distribution of Daily Mean Wind Speed", xlab = "Mean Wind Speed in MPH")
...
ggplot(tt, aes(x = Start.Station, y = End.Station, fill = Count)) + geom_tile() + scale_fill_gradient(low = "white", high = "steelblue") + theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1, colour="black"), axis.text.y = element_text(colour = "black")) + ggtitle("Traffic Between Stations\nBay Area Bike Share, August 2015")


#histogram of trip start & end times
par(mfrow=c(1,1), cex.main = 1)
hist(tripdata_final$StartTimeMin, main = "Rides By Daily Start Time",xaxt='n')
...
```
```{r, echo=FALSE, cache=TRUE, fig.width=9, fig.height=5.5}
