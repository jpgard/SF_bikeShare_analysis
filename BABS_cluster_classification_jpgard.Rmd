...
hc.avg = hclust(dist(tripdata_knn), method="single")

par(mfrow=c(1,3))
plot(hc.complete,main="Complete Linkage With Scaled Features", xlab="", sub="", cex =.1)
plot(hc.avg, main="Average Linkage With Scaled Features", xlab="", sub="", cex =.1)
plot(hc.avg, main="Single Linkage With Scaled Features", xlab="", sub="", cex =.1)

```

One import result that is readily visible from the dendograms above is the relatively balanced size of the clusters yielded by boyh the complete and the average linkage. We see lower-level clusters of approximately the same size  being progressively linked with similar-sized trips throughout the dendogram (besides the trips at the extreme right and left). In contrast, the single linkage displays much more unbalanced clusters, with mich larger and smaller clusters scattered throughout the dendogram. While the actual best clustering method would depend substantially on the underlying structure of the data, here, where we are interested primarily in exploratory analysis, more balanced clusters might be useful
